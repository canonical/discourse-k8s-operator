...
from charm import SERVICE_PORT, SERVICE_NAME
...
async def test_setup_discourse(ops_test: OpsTest, app: Application, app_config: dict[str, str]):
    ...
    form_fields = {
        ...
        "authenticity_token": parsed_registration.body.find("input", attrs={"name": "authenticity_token"})
                                                        .get("value"),
        "username": "admin",
    ...
    form_fields = {
        "_method": "put",
        "authenticity_token": parsed_validation.body.find("input", attrs={"name": "authenticity_token"}).get("value"),
    ...
    logger.info(f"Admin API Key: {parsed_key['key']['key']}")
...
async def test_s3_conf(ops_test: OpsTest, app: Application, s3_ip_address: str):
    ...
    action = await app.units[0].run(f"echo \"{s3_ip_address}  {s3_bucket}.{s3_domain}\" >> /etc/hosts")
