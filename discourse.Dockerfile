...
# Run build process.
RUN echo "Building ${CONTAINER_APP_NAME} ${CONTAINER_APP_VERSION} in ${CONTAINER_APP_ROOT}..."

RUN mkdir -p ${GEM_HOME}
RUN chown ${CONTAINER_APP_USERNAME}:${CONTAINER_APP_GROUP} ${GEM_HOME}

RUN cd ${CONTAINER_APP_ROOT}
RUN git clone --depth 1 --branch ${CONTAINER_APP_VERSION} https://github.com/discourse/discourse.git ${CONTAINER_APP_VERSION}

# Apply patch for LP#1903695
RUN git -C "${CONTAINER_APP_VERSION}" apply /srv/build_scripts/lp1903695.patch

RUN chown -R ${CONTAINER_APP_USERNAME}:${CONTAINER_APP_GROUP} ${CONTAINER_APP_VERSION}
RUN ln -s ${CONTAINER_APP_ROOT}/${CONTAINER_APP_VERSION} app
RUN cd ${CONTAINER_APP_ROOT}/app
...
RUN su -s /bin/bash -c 'bin/bundle install' ${CONTAINER_APP_USERNAME} 
...
RUN chown -R ${CONTAINER_APP_USERNAME}:${CONTAINER_APP_GROUP} ${CONTAINER_APP_ROOT}/app/*
