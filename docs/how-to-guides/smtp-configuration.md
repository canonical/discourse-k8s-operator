...
If authentication is needed, `smtp_authentication` will need to be set to the appropriate authentication method, accepted values are `none`, `login`, `plain`, `cram_md5`. The credencials can be set with `smtp_username`and `smtp_password`.
